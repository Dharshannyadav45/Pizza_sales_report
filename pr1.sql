...
select round(sum(order_details.quantity * pizzas.price),2) as revenue_generated
...
select category, count(name) as count_of_pizza from pizza_types
group by category;
...
SELECT pizza_types.category, 
       round(SUM(order_details.quantity * pizzas.price) / 
        (SELECT ROUND(SUM(order_details.quantity * pizzas.price), 2) AS revenue_generated
         FROM order_details
         JOIN pizzas ON pizzas.pizza_id = order_details.pizza_id) * 100,2) AS percentage_revenue
FROM pizza_types
